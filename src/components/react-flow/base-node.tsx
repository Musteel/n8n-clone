...
export function BaseNode({ className, status, ...props }: BaseNodeProps) {
  return (
    ...
    >
      {props.children}
      {status === "success" && (
        <CheckCircle2Icon className="absolute right-0.5 bottom-0.5 size-2 text-green-700 stroke-3 animate-pulse" />
      )}
      {status === "error" && (
        <XCircleIcon className="absolute right-0.5 bottom-0.5 size-2 text-red-700 stroke-3 animate-pulse" />
      )}
      {status === "loading" && (
